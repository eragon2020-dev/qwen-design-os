# Generate Product for Laravel Inertia 3 with React

This command generates a complete Laravel Inertia 3 export package with React frontend automatically, without asking for framework selection.
...
"To generate your Laravel Inertia React product, you need at minimum:
...
Continuing with Laravel Inertia React generation without these..."

## Execute Export with Inertia React Target

Execute the full export process from `/export-product` with the following pre-configured settings:

**TARGET_FRAMEWORK = inertia**
**INERTIA_FRONTEND = react**

This means:
- Generate Laravel RESTful controllers
- Generate Inertia page components with React 18+ Hooks
- Generate reusable React components
- Generate TypeScript type definitions
- Generate Laravel Inertia-specific instructions and prompts
- Create Laravel Inertia React handoff documentation

## Generate All Export Files

Follow all steps from the `/export-product` command for Laravel Inertia 3 with React:

1. **Create Directory Structure** — Laravel Inertia format
2. **Generate product-overview.md** — With Inertia React export format section
...
5. **Transform Components** — Generate controllers and React page components from React screen designs
...
10. **Generate Prompt Files** — Laravel Inertia React one-shot and section prompts
11. **Generate README.md** — Laravel Inertia React handoff guide
...
"I've generated the complete **Laravel Inertia 3 with React** export package at `product-plan/` and `product-plan.zip`.
...
- `shell/` — Inertia layout components (React)
- `sections/` — [N] section packages with controllers, Inertia pages (React), components, and test specs
...
The Inertia pages follow Laravel conventions — controllers return `Inertia::render()` with props, and pages receive data through props. React components use Hooks and functional components. How you architect Eloquent relationships, business logic, and authorization is up to you."
...
- Use the pre-written prompts — they prompt for important Laravel and React questions
...
- Inertia pages are portable — they work with any Laravel 13+ + Inertia v3 + React setup
...
- Use TypeScript types from `types.ts` for consistent React typing
