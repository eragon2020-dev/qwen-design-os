# Generate Product for Laravel Inertia 3 with Svelte

This command generates a complete Laravel Inertia 3 export package with Svelte frontend automatically, without asking for framework selection.
...
"To generate your Laravel Inertia Svelte product, you need at minimum:
...
Continuing with Laravel Inertia Svelte generation without these..."

## Execute Export with Inertia Svelte Target

Execute the full export process from `/export-product` with the following pre-configured settings:

**TARGET_FRAMEWORK = inertia**
**INERTIA_FRONTEND = svelte**

This means:
- Generate Laravel RESTful controllers
- Generate Inertia page components with Svelte 4+
- Generate reusable Svelte components
- Generate TypeScript type definitions
- Generate Laravel Inertia-specific instructions and prompts
- Create Laravel Inertia Svelte handoff documentation

## Generate All Export Files

Follow all steps from the `/export-product` command for Laravel Inertia 3 with Svelte:

1. **Create Directory Structure** — Laravel Inertia format
2. **Generate product-overview.md** — With Inertia Svelte export format section
...
5. **Transform Components** — Generate controllers and Svelte page components from React screen designs
...
10. **Generate Prompt Files** — Laravel Inertia Svelte one-shot and section prompts
11. **Generate README.md** — Laravel Inertia Svelte handoff guide
...
"I've generated the complete **Laravel Inertia 3 with Svelte** export package at `product-plan/` and `product-plan.zip`.
...
- `shell/` — Inertia layout components (Svelte)
- `sections/` — [N] section packages with controllers, Inertia pages (Svelte), components, and test specs
...
The Inertia pages follow Laravel conventions — controllers return `Inertia::render()` with props, and pages receive data through props. Svelte components use stores and reactive statements. How you architect Eloquent relationships, business logic, and authorization is up to you."
...
- Use the pre-written prompts — they prompt for important Laravel and Svelte questions
...
- Inertia pages are portable — they work with any Laravel 13+ + Inertia v3 + Svelte setup
...
- Use TypeScript types from `types.ts` for consistent Svelte typing
