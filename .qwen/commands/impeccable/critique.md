...
Before running this critique, ensure you have design context for the project. Check `.impeccable.md` in the project root or the loaded instructions for a **Design Context** section. If no design context exists, run `/teach-impeccable` first. Also gather: what the interface is trying to accomplish.
...
- **Cognitive load check**: Run the 8-item checklist — working memory limits, chunking, recognition over recall, clear labels, visible options, progressive disclosure, consistent patterns, error prevention. Report failure count: 0–1 = low (good), 2–3 = moderate, 4+ = critical.
...
- **P0 Critical**: Blocks core tasks, fundamental UX failure
- **P1 Major**: Significant friction, users struggle but can complete
...
Auto-select 2–3 personas most relevant to this interface type. If `.impeccable.md` contains a `## Design Context` section from `/teach-impeccable`, also generate 1–2 project-specific personas from the audience/brand info.
...
**After presenting findings**, use targeted questions based on what was actually found. These answers will shape the action plan.
